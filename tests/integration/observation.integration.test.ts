...
describe('Observation Tool Integration Tests', () => {
  ...
  describe('createObservation', () => {
    it('should create a new observation with valueQuantity successfully', async () => {
      if (!testPatient?.id || !testEncounter?.id || !testPractitioner?.id)
        throw new Error('Test entities not initialized');
    ...
    it('should create a new observation with valueCodeableConcept successfully', async () => {
      ...
      const obsArgs: CreateObservationArgs = {
        status: 'final',
        code: {
          coding: [{ system: snomedSystem, code: '165040000', display: 'Finding of tobacco smoking behavior' }],
          text: 'Smoking Status',
        },
        subjectId: testPatient.id,
        effectiveDateTime: new Date().toISOString(),
    ...
    it('should throw an error if required fields are missing (e.g., subjectId)', async () => {
      ...
      await expect(createObservation(observationArgs)).rejects.toThrow(
        'Patient reference is required to create an observation.',
      );
    });
    ...
    it('should throw an error if code is missing', async () => {
      ...
      await expect(createObservation(observationArgs)).rejects.toThrow(
        'Observation code with at least one coding is required.',
      );
    });
  ...
  describe('updateObservation', () => {
    ...
    beforeEach(async () => {
      ...
      const obsArgs: CreateObservationArgs = {
        ...
        code: {
          coding: [{ system: loincSystem, code: '8310-5', display: 'Body temperature' }],
  ...
  describe('searchObservations', () => {
    let obs1: Observation | null,
      obs2: Observation | null,
      obs3_vitals: Observation | null,
      obs4_bp_component_systolic: Observation | null,
      obs5_bp_component_diastolic: Observation | null;
    ...
    beforeAll(async () => {
      ...
      obs3_vitals = await createObservation({
        status: 'final',
        code: {
          coding: [{ system: loincSystem, code: '20564-1', display: 'Oxygen saturation' }],
      ...
      obs4_bp_component_systolic = await createObservation({
        status: 'final',
        code: {
          coding: [{ system: loincSystem, code: '8480-6', display: 'Systolic blood pressure' }],
      ...
      obs5_bp_component_diastolic = await createObservation({
        status: 'final',
        code: {
          coding: [{ system: loincSystem, code: '8462-4', display: 'Diastolic blood pressure' }],
      ...
      if (!obs1 || !obs2 || !obs3_vitals || !obs4_bp_component_systolic || !obs5_bp_component_diastolic)
        throw new Error('Failed to create observations for search tests');
      console.log(
        `Created observations for search tests: ${obs1.id}, ${obs2.id}, ${obs3_vitals.id}, ${obs4_bp_component_systolic.id}, ${obs5_bp_component_diastolic.id}`,
      );
    });
    ...
    it('should find observations by patient ID and code', async () => {
      ...
      expect(results.every((o) => o.code?.coding?.some((c) => c.code === bpCode && c.system === loincSystem))).toBe(
        true,
      );
    });
    ...
    it('should find observation by identifier', async () => {
      const results = await searchObservations({
        identifier: `${obs1!.identifier![0].system}|${uniqueIdentifierSearch1}`,
      });
    ...
    it('should warn and return results or empty for search with no criteria', async () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining('Observation search called with no specific criteria'),
      );
      expect(results).toBeInstanceOf(Array);
