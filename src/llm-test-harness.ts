...
class MCPChatTestHarness {
  ...
  async processUserQuery(query: string): Promise<string> {
    ...
    try {
      const maxIterations = 3; // Reduced to encourage asking for help sooner
          ...
          for (const toolCall of assistantMessage.tool_calls) {
            ...
            try {
              ...
              if (resultStr.includes('"error":') && resultStr.includes('"success": false')) {
                consecutiveFailures++;
                toolResults.push(
                  `Tool ${toolName} failed: ${resultStr}. ${consecutiveFailures >= 2 ? 'MULTIPLE FAILURES DETECTED - Consider asking the user for clarification about required parameters.' : ''}`,
                );
              } else {
              ...
              consecutiveFailures++;
              toolResults.push(
                `Tool ${toolName} error: ${error}. ${consecutiveFailures >= 2 ? 'MULTIPLE FAILURES DETECTED - Consider asking the user for clarification about required parameters.' : ''}`,
              );
            }
          }
